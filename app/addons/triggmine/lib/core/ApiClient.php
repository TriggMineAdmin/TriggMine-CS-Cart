	...
	final class TriggMine_ApiClient
	{
		...
		private $_actionMapping = array(
			...
			'updateBuyer'      => array(
				...
				'paramsOut' => array(self::KEY_BUYER_ID, self::KEY_BUYER_EMAIL, self::KEY_BUYER_REG_START, self::KEY_BUYER_REG_END, self::KEY_BUYER_BIRTHDAY)
			),
		...
		public function request($request, &$response = null)
		{
			...
			try {
				...
				if ($this->_integrator->supportsJavaScript()
					...
					$url = strstr($this->_apiUrl, 'https://') ? $this->_apiUrl : 'https://' . $this->_apiUrl;
					$url .= substr($url, -1) === '/' ? '' : '/';
